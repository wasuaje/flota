...
class AutocompleteWidget(object):    
    def __init__(self, request, field, id_field=None, db=None, orderby=None, limitby=(0,10), keyword='_autocomplete_%(fieldname)s', min_length=2):	
        ...
        #with the adition of         self.keyword = keyword % dict(fieldname=field._tablename+'_'+field.name)
        #instead of self.keyword = keyword % dict(fieldname=field.name)
        #solved problem with multiple widget on different tables with same field name
	#Wuelfhis Asuaje
        self.keyword = keyword % dict(fieldname=field._tablename+'_'+field.name)
    ...
    def __call__(self,field,value,**attributes):
        ...
        if self.is_reference:
            ...
            attr['_onblur']="jQuery('#%(div_id)s').delay(100).fadeOut('slow');" % \
